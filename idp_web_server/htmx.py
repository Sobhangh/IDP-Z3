...
from idp_engine.Expression import TRUE, FALSE, BOOL_SETNAME, INT_SETNAME, REAL_SETNAME
...
def ass_head(ass, state, id=None):
    ...
    if ass.sentence.type == BOOL_SETNAME:
        yield span([
        ...
        yield span(hx_trigger="uncollapse", hx_swap="none",
                   ...
                   i=[sanitize(ass.sentence.code),
                      ...
                      info() ])
    elif ass.sentence.type in [INT_SETNAME, REAL_SETNAME]:
        yield span([
            sanitize(ass.sentence.code),
